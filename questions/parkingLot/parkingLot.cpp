...
public:
    static Vehicle* createVehicle(VehicleType type, string plate) {
        if (type == VehicleType::BIKE)  return new Bike(plate);
        if (type == VehicleType::CAR)   return new Car(plate);
        if (type == VehicleType::TRUCK) return new Truck(plate);
        return nullptr;
    }
};
...
public:
    ...
    bool canFitVehicle(VehicleType vt) {
        ...
        if (vt == VehicleType::CAR) return size == SpotSize::MEDIUM || size == SpotSize::LARGE;
...
public:
    double calculateFare(SpotSize size, int hours) override {
        int rate = (size == SpotSize::SMALL) ? 10 : (size == SpotSize::MEDIUM) ? 20 : 30;
        return rate * hours;
    }
};
...
public:
    ...
    Vehicle* vehicle;
    ParkingSpot* spot;
    int entryHour; // just a number like 10 (10 AM), easy to write, no time_t needed
...
public:
    ...
    // find + assign together under one lock, so the check and the claim are atomic
...
public:
    ...
    double unparkVehicle(int ticketId, int exitHour) {
        ...
        double fare = pricingStrategy->calculateFare(t->spot->getSize(), hoursParked);

        t->spot->removeVehicle();
        activeTickets.erase(ticketId);

        cout << "Vehicle " << t->vehicle->getPlate() << " exited. Fare: Rs " << fare << endl;
        return fare;
    }
};
...
int main() {
    ...
    Vehicle* bike1 = VehicleFactory::createVehicle(VehicleType::BIKE, "KA-01-9999");
